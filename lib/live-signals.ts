...
export async function getLiveCloudAlerts({ id, state, limit = 50, timeoutMs = 8_000 }: { id?: string | null; state?: CloudLifecycleState | null; limit?: number; timeoutMs?: number } = {}) {
  const safeLimit = Math.max(1, Math.min(100, Math.trunc(limit)));
