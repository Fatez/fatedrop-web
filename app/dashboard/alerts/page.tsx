...
function SignalCard({ signal, unlocked, now }: { signal: NetworkSignal; unlocked: boolean; now: number }) {
  ...
  const title = unlocked ? signal.title : "Premium signal detail";
  const retailer = unlocked ? (signal.retailer || "Retailer pending") : "Retailer hidden";
  const detail = unlocked ? (signal.detail || meta.description) : "Unlock Premium to reveal the product, retailer and actionable signal context.";
  const delivered = unlocked ? money(signal.deliveredPricePence) : "£—.——";
  ...
  return (
    <article className={`fd-alert-row state-${signal.state} ${unlocked ? "" : "locked"}`}>
      ...
      <div className="fd-alert-body">
        <div className="fd-alert-titleline"><div className={unlocked ? "" : "fd-alert-blur"}><strong>{title}</strong><small>{retailer}</small></div><time>{relativeTime(signal.occurredAt, now)}</time></div>
        <p className={unlocked ? "" : "fd-alert-blur fd-alert-detail"}>{detail}</p>
        <div className="fd-alert-facts"><span><small>DELIVERED</small><b className={unlocked ? "" : "fd-alert-blur"}>{delivered}</b></span><span><small>STATE</small><b>{meta.label}</b></span><span><small>DETECTED</small><b>{relativeTime(signal.occurredAt, now)}</b></span></div>
      </div>
  ...
  return (
    <DashboardPageShell title="Alerts" eyebrow="LIVE SIGNAL NETWORK">
      <div className="fd-alerts-content">
        ...
        {!unlocked ? <section className="fd-alerts-gate"><div><span>PREMIUM SIGNAL INTELLIGENCE</span><h2>The feed is active. Actionable details are locked.</h2><p>{plan} access can see the signal state, while Premium reveals product, retailer and price intelligence.</p></div>{hasOpenSubscription ? <Link className="button button-primary" href="/dashboard/membership">Manage membership →</Link> : <StartMembershipButton tier="plus" label={trialEligible ? "Start free trial" : snapshot?.membership.stripeCustomerId ? "Restart Plus" : "Choose Plus"} />}</section> : null}
