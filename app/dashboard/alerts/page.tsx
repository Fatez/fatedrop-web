...
function observedDurationLabel(seconds: number | null) {
...
function companionVoice(stage: CanonicalSignalStage) {
  ...
  if (stage === "VANISHED") return "Nyxen saw the verified availability disappear";
  ...
  return <DashboardPageShell title="Alerts" eyebrow="YOUR HUNTS · YOUR NOTIFICATIONS">
    <div className="fd-personal-alerts">
      <section className="fd-alert-personal-hero">
        <div><span>FATEDROP // PERSONAL ALERT CENTRE</span><h1>Your friends are hunting<br/><em>the network for you.</em></h1><p>Oru watches for Whispers, Fenn catches Echoes, Koru confirms Manifested stock and Nyxen tracks what Vanishes. The dashboard, app and Discord all follow the same canonical FateDrop lifecycle and the same underlying retailer evidence.</p><div className="fd-alert-hero-actions"><Link href="/dashboard/watchlist">Create FateFind →</Link><Link href="/dashboard/notifications">Notification preferences →</Link><Link href="/dashboard">Open Network Activity →</Link></div></div>
      ...
      <section className="fd-dash-card fd-canonical-alerts"><div className="fd-dash-card-head"><span>CANONICAL SIGNAL INBOX</span><small>{canonicalAlerts.length ? `${canonicalAlerts.length} recent` : "Awaiting signals"}</small></div>{canonicalAlerts.length ? <div className="fd-canonical-list">{canonicalAlerts.map((alert)=>{
        ...
        return <article key={alert.id} className={`fd-canonical-signal ${alert.fateStage.toLowerCase()}`}>
          ...
          <div className="fd-canonical-copy"><strong>{alert.title}</strong><span>{premium ? `${companionVoice(alert.fateStage)} · ${alert.retailer} · ${alert.message}` : `${companionVoice(alert.fateStage)} · Connected retailer · Premium price detail`}</span>{alert.fateStage === "VANISHED" && observedDurationLabel(alert.observedDurationSeconds) ? <em className="observed">OBSERVED LIVE · {observedDurationLabel(alert.observedDurationSeconds)}</em> : null}{premium && rrpLine(alert) ? <em>{rrpLine(alert)}</em> : null}{premium ? <em className={`verdict ${verdict.tone}`}>{verdict.text}</em> : null}</div>
      ...
      <div className="fd-alert-personal-grid">
        ...
        <section className="fd-dash-card fd-alert-delivery"><div className="fd-dash-card-head"><span>DELIVERY CHANNELS</span><Link href="/dashboard/notifications">Edit preferences</Link></div><div className="fd-delivery-list"><div><b>WEB</b><span>Canonical signal inbox plus account notification history.</span><i className="live">AVAILABLE</i></div><div><b>DISCORD</b><span>Oru, Fenn, Koru and Nyxen route the same canonical lifecycle into their dedicated Discord channels when configured.</span><i className="pending">CONFIGURATION-DEPENDENT</i></div><div><b>APP PUSH</b><span>Device registration and exact-alert routing are connected; remote delivery remains feature-gated until controlled production testing.</span><i className="pending">VALIDATING</i></div></div><p>Oru/Whisper, Fenn/Echo, Koru/Manifested, Nyxen/Vanished, price and FateMatch preferences share one account-level persistence model. FateDrop never treats an unconfigured channel as delivered.</p></section>
      </div>
