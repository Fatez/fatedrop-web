  ...
  return <DashboardPageShell title="Alerts" eyebrow="YOUR HUNTS · YOUR NOTIFICATIONS">
    <div className="fd-personal-alerts">
      <section className="fd-alert-personal-hero">
        <div><span>FATEDROP // PERSONAL ALERT CENTRE</span><h1>Network activity is global.<br/><em>Alerts are yours.</em></h1><p>Alerts focuses on the things FateDrop is watching or has delivered for you: active FateFind hunts, successful FateMatch activity and your notification history. The global network heartbeat belongs on Home.</p><div className="fd-alert-hero-actions"><Link href="/dashboard/watchlist">Create FateFind →</Link><Link href="/dashboard/notifications">Notification preferences →</Link><Link href="/dashboard">Open Network Activity →</Link></div></div>
      ...
      <div className="fd-alert-personal-grid">
        ...
        <section className="fd-dash-card fd-alert-delivery"><div className="fd-dash-card-head"><span>DELIVERY CHANNELS</span><Link href="/dashboard/notifications">Edit preferences</Link></div><div className="fd-delivery-list"><div><b>WEB</b><span>Dashboard notification history and account activity.</span><i className="live">AVAILABLE</i></div><div><b>DISCORD</b><span>Shared account preference; delivery activates only when Discord is linked, enabled and entitled.</span><i className="pending">CONFIGURATION-DEPENDENT</i></div><div><b>APP PUSH</b><span>Shared account preference ready for the mobile delivery integration to consume.</span><i className="pending">FOUNDATION</i></div></div><p>Echo, Manifested, Vanished, price and FateMatch preferences now have one account-level persistence model. Channel delivery still remains evidence/configuration dependent: saving a preference does not pretend that an unconnected push or Discord service is live.</p></section>
      </div>
