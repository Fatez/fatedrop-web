  ...
  return <DashboardPageShell title="Alerts" eyebrow="NETWORK FLIGHT RECORDER">
    <div className="fd-ledger-page">
      <section className="fd-ledger-hero fd-dash-card">
        ...
        <div className="fd-ledger-stages">{lifecycle.map((key) => <Link className={stage === key ? `active ${key.toLowerCase()}` : key.toLowerCase()} key={key} href={filterHref({ stage: stage === key ? undefined : key, cause: cause ?? undefined, q: q || undefined })}><span>{key}</span><strong>{stageCounts[key]}</strong><small>{key === "WHISPER" ? "network movement detected" : key === "ECHO" ? "retailer readiness detected" : key === "MANIFESTED" ? "stock verified live" : "stock verified gone"}</small></Link>)}</div>
      </section>
