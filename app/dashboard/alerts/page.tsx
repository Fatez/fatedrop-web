...
export default async function AlertsPage() {
  ...
  const initialSignals = unlocked ? signals : signals.map((signal) => ({
    ...signal,
    confidence: null,
  ...
  const counts = {
    whisper: signals.filter((item) => (item.kind ?? item.state) === "whisper").length,
    manifested: signals.filter((item) => (item.kind ?? item.state) === "manifested").length,
    vanished: signals.filter((item) => (item.kind ?? item.state) === "vanished").length,
    echo: signals.filter((item) => (item.kind ?? item.state) === "echo").length,
  };
