...
function filterHref(input: { tcg?: TcgCode; stage?: string; cause?: string; q?: string }) {
  const params = new URLSearchParams();
  if (input.tcg) params.set("tcg", input.tcg);
...
export default async function AlertsPage({ searchParams }: { searchParams: Promise<{ tcg?: string; stage?: string; cause?: string; q?: string }> }) {
  ...
  const tcg = isTcgCode(params.tcg) && selectedTcgCodes.includes(params.tcg) ? params.tcg : null;
  ...
  const tcgAlerts = tcg ? alerts.filter((alert) => alert.tcgCode === tcg) : alerts.filter((alert) => selectedTcgCodes.includes(alert.tcgCode as TcgCode));
  const stageCounts = Object.fromEntries(lifecycle.map((key) => [key, tcgAlerts.filter((alert) => alert.fateStage === key).length])) as Record<FilterStage, number>;
  const filtered = tcgAlerts.filter((alert) => {
  ...
  const activeFateMatchWatches = fateMatchWatches.filter((item) => item.enabled && (!tcg || item.tcgCode === tcg));
  ...
  return <DashboardPageShell title="Alerts" eyebrow="NETWORK FLIGHT RECORDER">
    <div className="fd-ledger-page">
      <section className="fd-ledger-hero fd-dash-card">
        <div className="fd-ledger-intro"><span>PRECISE SIGNAL ACTIVITY</span><h1>Every signal should tell you exactly what the network observed.</h1><p><b>Lifecycle</b> tells you where the opportunity is: Whisper, Echo, Manifested or Vanished. <b>Cause</b> tells you why that record exists: catalogue change, queue, security, preparation, restock, sold out and so on. They are recorded separately so the dashboard never turns every bit of activity into the same alarm.</p></div>
        <nav className="fd-ledger-tcgs" aria-label="Filter alerts by trading card game"><Link className={!tcg ? "active" : ""} href={filterHref({ stage: stage ?? undefined, cause: cause ?? undefined, q: q || undefined })}>ALL SELECTED</Link>{TCG_REGISTRY.filter((entry) => selectedTcgCodes.includes(entry.code)).map((entry) => <Link key={entry.code} className={tcg === entry.code ? "active" : ""} href={filterHref({ tcg: entry.code, stage: stage ?? undefined, cause: cause ?? undefined, q: q || undefined })}>{entry.shortName}{entry.live ? "" : " · SOON"}</Link>)}</nav>
        <div className="fd-ledger-stages">{lifecycle.map((key) => <Link className={stage === key ? `active ${key.toLowerCase()}` : key.toLowerCase()} key={key} href={filterHref({ tcg: tcg ?? undefined, stage: stage === key ? undefined : key, cause: cause ?? undefined, q: q || undefined })}><span>{key}</span><strong>{stageCounts[key]}</strong><small>{key === "WHISPER" ? "network movement detected" : key === "ECHO" ? "retailer readiness detected" : key === "MANIFESTED" ? "stock verified live" : "stock verified gone"}</small></Link>)}</div>
      </section>
      ...
      <section className="fd-ledger-filter fd-dash-card">
        <form action="/dashboard/alerts" method="get">
          ...
          {tcg ? <input type="hidden" name="tcg" value={tcg}/> : null}
          ...
          {(stage || cause || q || tcg) ? <Link href="/dashboard/alerts">CLEAR</Link> : null}
        </form>
      ...
      <section className="fd-ledger-card fd-dash-card">
        ...
        {filtered.length ? <div className="fd-ledger-list">{filtered.map((alert) => {
          ...
          return <article className={`fd-ledger-row ${stageClass}`} key={alert.id}>
            <div className="fd-ledger-state"><i/><b>{alert.fateStage}</b><em>{TCG_REGISTRY.find((entry) => entry.code === alert.tcgCode)?.shortName ?? alert.tcgCode} · {exactCause.label}</em><small>{relativeTime(alertTime(alert), generatedAt)}</small></div>
            ...
            <div className="fd-ledger-actions">{premium ? <a className="primary" href={alert.productUrl} target="_blank" rel="noreferrer">{primaryActionLabel(alert.fateStage)}</a> : <Link className="primary" href="/dashboard/membership">UNLOCK →</Link>}<Link href={`/dashboard/fatefind?tcg=${alert.tcgCode}&q=${encodeURIComponent(alert.preparedLinks.compareQuery)}`}>FATEFIND</Link><Link href={`/dashboard/watchlist?tcg=${alert.tcgCode}&q=${encodeURIComponent(alert.preparedLinks.fateFindQuery)}`}>FATEMATCH</Link></div>
