    ...
    <DashboardPageShell title="Discord" eyebrow="COMMUNITY + PREMIUM ACCESS">
      <div className="fd-discord-page">
        <section className="fd-dash-card fd-discord-hero">
          <div className="fd-dash-card-head"><span>FATEDROP DISCORD</span><i className={DISCORD_COMMUNITY_OPEN ? "live" : "pending"}>{DISCORD_COMMUNITY_OPEN ? "● COMMUNITY OPEN" : "○ COMMUNITY HELD"}</i></div>
          <div className="fd-discord-hero-grid">
            <div><span className="fd-discord-kicker">ONE IDENTITY · CONFIGURATION AWARE</span><h1>{DISCORD_COMMUNITY_OPEN ? <>Everyone can join.<br/><em>Premium intelligence stays gated.</em></> : <>Identity linking is built.<br/><em>Public invites are not open yet.</em></>}</h1><p>{DISCORD_COMMUNITY_OPEN ? "The FateDrop Discord community is open to members while Premium signal channels remain entitlement-gated." : "The Discord identity and Premium-role foundations exist, but FateDrop will not advertise the public community as open until the deployment flag and server configuration say it is."}</p></div>
            <div className="fd-discord-access-card"><span>YOUR ACCESS</span><strong>{premium ? "PREMIUM ENTITLEMENT" : "FREE ENTITLEMENT"}</strong><small>{plan}</small><div className="fd-discord-access-line"><i data-on={DISCORD_COMMUNITY_OPEN ? "true" : "false"}/>{DISCORD_COMMUNITY_OPEN ? "Public community available" : "Public community not opened"}</div><div className="fd-discord-access-line"><i data-on={premium ? "true" : "false"}/>Premium role eligibility</div></div>
          </div>
        </section>

        <div className="fd-discord-steps">
          <section className="fd-dash-card fd-discord-step"><span className="fd-discord-number">01</span><small>JOIN THE SERVER</small><h2>{DISCORD_COMMUNITY_OPEN ? "Enter the FateDrop community." : "Community invite is held."}</h2><p>{DISCORD_COMMUNITY_OPEN ? "Joining Discord is free. This gives access to the public community areas only; it does not grant Premium channels." : "The website will expose the real invite only when the community-open deployment flag is enabled. No dead or placeholder invite is presented as live."}</p>{DISCORD_COMMUNITY_OPEN ? <a className="fd-dashboard-wide-button" href={DISCORD_INVITE_URL} target="_blank" rel="noreferrer">Join FateDrop Discord →</a> : <span className="fd-discord-held">Community invites currently held</span>}</section>

          <section className="fd-dash-card fd-discord-step"><span className="fd-discord-number">02</span><small>LINK YOUR IDENTITY</small><h2>{linked ? "Discord linked to FateDrop." : "Connect Discord to your account."}</h2><p>{linked ? "Your Discord identity is linked. When the server integration is configured, FateDrop can keep its role aligned with your membership." : "Linking tells FateDrop which Discord identity belongs to this account. Linking alone never grants Premium access."}</p>{linked ? <div className="fd-discord-actions"><DiscordSyncButton/><DiscordUnlinkButton/></div> : <a className="button button-primary" href="/api/discord/connect">Link Discord account ↗</a>}</section>

          <section className="fd-dash-card fd-discord-step fd-discord-premium"><span className="fd-discord-number">03</span><small>PREMIUM ROLE</small><h2>{premium ? "Your membership qualifies." : "Premium role is locked."}</h2><p>{premium ? "Your FateDrop membership is eligible for the configured Premium role. Role application still depends on the Discord bot, guild and role being correctly configured." : "A Premium membership or eligible trial is required for Premium-role access. Free members remain part of the public community when it is open."}</p>{premium ? (linked ? <DiscordSyncButton/> : <a className="button button-primary" href="/api/discord/connect">Link Discord identity →</a>) : <Link className="fd-dashboard-wide-button" href="/dashboard/membership">View membership options →</Link>}</section>
        </div>
      </div>
      <style>{`
