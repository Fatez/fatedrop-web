    ...
    <div className="fd-command-centre">
      <section className="fd-command-hero">
        <div className="fd-command-copy">
          ...
          <p>{latestSignal ? `${signalLabel(latestSignal)} detected ${relativeTime(latestSignal.occurredAt, data.generatedAt)}${latestSignal.retailer ? ` at ${latestSignal.retailer}` : ""}.` : "FateDrop is watching connected evidence. When catalogue, access or availability changes are persisted, the signal resolves here without inventing activity while the network is quiet."}</p>
          <div className="fd-command-actions"><Link href="/dashboard/search">Search the network →</Link><Link href="/dashboard/alerts">Open signal feed →</Link></div>
        </div>
        ...
        <div className="fd-command-metrics"><span><b>{metric(data.publicSignalMetrics.whisper)}</b>WHISPER</span><span><b>{metric(data.publicSignalMetrics.echo)}</b>ECHO</span><span><b>{metric(data.publicSignalMetrics.manifested)}</b>MANIFESTED</span><span><b>{metric(network?.metrics.changes24h)}</b>24H CHANGES</span><span><b>{plan}</b>YOUR PLAN</span></div>
      </section>
      ...
      <section className="fd-core-actions" aria-label="Core FateDrop actions">
        ...
        <Link href="/dashboard/alerts"><span>◉</span><div><small>DETECT</small><strong>Live Signals</strong><p>Whisper, Echo, Manifested and Vanished lifecycle intelligence from observed transitions.</p></div><b>→</b></Link>
