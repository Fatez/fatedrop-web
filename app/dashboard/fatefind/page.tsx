...
export default async function DashboardFateFindPage({ searchParams }: { searchParams: Promise<{ q?: string; tcg?: string }> }) {
  ...
  const result = q.length >= 2 && tcgDefinition.live ? await searchSignalFateVerdict(q,{tcgCode:tcg}) : null;
    ...
    <div className="fd-true-price-page">
      <section className="fd-dash-card fd-true-price-hero">
        ...
        <form action="/dashboard/fatefind" method="get" className="fd-tp-search">
          <select name="tcg" defaultValue={tcg} aria-label="Trading card game">{TCG_REGISTRY.filter((entry)=>selectedTcgCodes.includes(entry.code)).map((entry)=><option key={entry.code} value={entry.code} disabled={!entry.live}>{entry.shortName}{entry.live?"":" · coming soon"}</option>)}</select>
      ...
      <section className="fd-dash-card fd-tp-results">
        <div className="fd-tp-results-head"><div><span>{q ? `FATEFIND · ${tcgDefinition.shortName.toUpperCase()} · ${q.toUpperCase()}` : `COMPARE ${tcgDefinition.shortName.toUpperCase()} OFFERS`}</span><h2>{q ? "Which live option is the strongest value?" : "Search once. Let FateFind compare the network."}</h2></div><small>{result ? `${result.count} product group${result.count === 1 ? "" : "s"}` : !tcgDefinition.live ? "Selected TCG is coming soon" : q.length >= 2 ? "Cloud response unavailable" : "Enter a product above"}</small></div>
        {!q ? <div className="fd-dashboard-empty"><strong>Type a product above.</strong><span>FateDrop compares qualifying observed retailer offers, RRP/reference position and known checkout cost without inventing missing evidence.</span></div> : q.length < 2 ? <div className="fd-dashboard-empty"><strong>Keep typing.</strong><span>Use at least two characters so FateDrop can resolve a meaningful product search.</span></div> : !result ? <div className="fd-dashboard-empty"><strong>The Signal Engine could not be reached.</strong><span>No sample prices or pretend retailers are substituted.</span></div> : groups.length ? <>
          ...
          <div className="fd-tp-groups">{groups.map((group) => {
            ...
            return <article className="fd-tp-group" key={group.id}>
              ...
              <footer><Link href={`/dashboard/watchlist?tcg=${encodeURIComponent(tcg)}&q=${encodeURIComponent(group.title)}`}>FATEMATCH · WATCH MY CONDITIONS →</Link><Link href={`/dashboard/search?tcg=${encodeURIComponent(tcg)}&q=${encodeURIComponent(group.title)}`}>OPEN NETWORK SEARCH →</Link></footer>
            </article>;
          })}</div>
        </> : <div className="fd-dashboard-empty"><strong>{tcgDefinition.live ? "No comparable live offers found." : `${tcgDefinition.shortName} catalogue and monitoring are not active yet.`}</strong><span>{tcgDefinition.live ? "FateDrop will not invent a comparison when the network has no qualifying observed offers." : "Your interest is saved, but FateDrop will not search, monitor or alert until Cloud activates verified catalogue truth for this TCG."}</span>{tcgDefinition.live?<Link className="fd-dashboard-wide-button" href={`/dashboard/watchlist?tcg=${encodeURIComponent(tcg)}&q=${encodeURIComponent(q)}`}>Create a FateMatch stock watch →</Link>:null}</div>}
