...
export const metadata: Metadata = { title: "FateFind & Watchlist | FateDrop Dashboard", robots: { index: false, follow: false } };
...
export default async function DashboardFateMatchPage() {
  ...
  return <DashboardPageShell title="FateFind" eyebrow="SAVED INTENT · NETWORK MONITORING">
    <div className="fd-fatematch-page">
      <section className="fd-dash-card fd-fm-hero">
        ...
        <div className="fd-fm-hero-grid"><div><div className="fd-network-message"><h1>Tell FateDrop what you want.<br/>Let the network do the hunting.</h1><p>FateFind is the structured saved-intent layer: product, price/RRP limits and online or local scope in one rule evaluated against real network evidence. Watchlist remains the simpler saved-product concept. Existing internal FateMatch storage names are retained for compatibility while the public product language is cleaned up.</p></div><FateMatchBuilder premium={premium}/></div>{avatar ? <aside className="fd-fm-companion"><div><span>YOUR COMPANION</span><small>Watching the network with you</small></div><AvatarPreview loadout={avatar.loadout} mood={matches.some((match)=>match.enabled) ? "watching" : "idle"} compact label="FateFind companion"/><a href="/dashboard/avatar">CUSTOMISE →</a></aside> : null}</div>
      </section>
      ...
      <div className="fd-fm-grid">
        ...
        <section className="fd-dash-card fd-fm-explain"><div className="fd-dash-card-head"><span>ONE SAVED-INTENT MODEL</span><small>Watchlist + FateFind</small></div><div className="fd-fm-flow"><span><b>1</b><strong>YOU SET THE RULE</strong><small>Product · price · RRP · online/local</small></span><i>→</i><span><b>2</b><strong>FATEDROP WATCHES</strong><small>Signals · catalogue · verified opportunities</small></span><i>→</i><span><b>3</b><strong>FATEFIND EXPLAINS WHY</strong><small>Structured evidence, not mystery alerts</small></span></div></section>
      </div>

      <section className="fd-dash-card fd-fm-legacy"><div className="fd-dash-card-head"><span>PREVIOUS SAVED-HUNT HITS</span><small>Retained history · not deleted</small></div><div className="fd-dashboard-list">{legacyHits.length && data ? legacyHits.map((item)=><article key={item.id}><span className="fd-store-thumb">◇</span><div><strong>{item.title || "Saved-hunt match"}</strong><small>{item.retailer || item.subtitle || "FateDrop activity"}</small></div><aside>{item.amountPence ? moneyFromPence(item.amountPence) : "HIT"}<small>{relativeTime(item.occurredAt,data.generatedAt)}</small></aside></article>) : <div className="fd-dashboard-empty"><strong>No previous saved-hunt hits.</strong><span>Nothing is being fabricated or migrated into FateFind without a real user intent record.</span></div>}</div></section>
    </div>
