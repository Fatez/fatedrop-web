...
function rrpDelta(delivered: number | undefined, rrp: number | undefined) {
  if (typeof delivered !== "number" || typeof rrp !== "number" || !Number.isFinite(delivered) || !Number.isFinite(rrp) || rrp <= 0) return null;
...
function sourceLabel(source: string | undefined) {
  if (!source) return "RRP source unavailable";
  if (source === "pokemon-center-uk") return "Pokémon Center UK observed RRP";
  return `Observed RRP source: ${source}`;
}
    ...
    <div className="fd-true-price-page">
      <section className="fd-dash-card fd-true-price-hero">
        <div className="fd-tp-intro">
          ...
          <p>True Price answers one simple question: <b>“What will this actually cost me?”</b> FateDrop keeps the item price and delivery separate, then adds them only when the delivery cost is genuinely known.</p>
        </div>
        <div className="fd-tp-simple">
          <div><b>1</b><strong>ITEM PRICE</strong><span>What the shop charges for the product.</span></div>
          <i>+</i>
          <div><b>2</b><strong>KNOWN DELIVERY</strong><span>Only a real mandatory delivery cost. Unknown never means free.</span></div>
          <i>=</i>
          <div className="answer"><b>3</b><strong>TRUE PRICE</strong><span>The known total you would actually pay.</span></div>
        </div>
        <form action="/dashboard/true-price" method="get" className="fd-tp-search">
          ...
          <button type="submit">COMPARE THE NETWORK →</button>
        </form>
        <p className="fd-tp-kid-copy">If Shop A says £50 + £5 delivery and Shop B says £53 with free delivery, Shop B is actually cheaper. True Price does that maths before you waste time opening every checkout.</p>
      </section>

      <section className="fd-dash-card fd-tp-results">
        <div className="fd-tp-results-head"><div><span>{q ? `COMPARING · ${q.toUpperCase()}` : "COMPARE OFFERS"}</span><h2>{q ? "Who is actually cheapest?" : "Search once. Compare the network."}</h2></div><small>{result ? `${result.count} product group${result.count === 1 ? "" : "s"}` : q.length >= 2 ? "Cloud response unavailable" : "Enter a product above"}</small></div>
        {!q ? <div className="fd-dashboard-empty"><strong>Type a product above.</strong><span>FateDrop will compare qualifying observed retailer offers instead of making you open a pile of tabs yourself.</span></div> : q.length < 2 ? <div className="fd-dashboard-empty"><strong>Keep typing.</strong><span>Use at least two characters so FateDrop can resolve a meaningful product search.</span></div> : !result ? <div className="fd-dashboard-empty"><strong>The Signal Engine could not be reached.</strong><span>No sample prices or pretend retailers are substituted.</span></div> : groups.length ? <div className="fd-tp-groups">{groups.map((group) => <article className="fd-tp-group" key={group.id}>
          <header><div><small>{group.category} · {group.retailerCount} RETAILER{group.retailerCount === 1 ? "" : "S"}</small><h2>{group.title}</h2><p>{typeof group.rrpGbp === "number" ? `${gbp(group.rrpGbp)} RRP · ${sourceLabel(group.rrpSource)}${group.rrpObservedAt ? ` · observed ${new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "short", year: "numeric" }).format(new Date(group.rrpObservedAt))}` : ""}` : "Verified RRP unavailable for this product identity"}</p></div><span>{Math.round(group.matchingConfidence * 100)}% identity confidence</span></header>
          <div className="fd-tp-offers">{[...group.offers].sort((a, b) => {
            ...
            const delta = rrpDelta(offer.totalDeliveredGbp, group.rrpGbp);
            return <div className={offer.isLowestKnownDelivered ? "fd-tp-offer best" : "fd-tp-offer"} key={offer.id}>
              ...
              <div className="fd-tp-cost"><span><small>ITEM</small><b>{gbp(offer.priceGbp)}</b></span><span><small>DELIVERY</small><b>{offer.deliveryKnown ? offer.shippingGbp === 0 ? "FREE" : gbp(offer.shippingGbp) : "UNKNOWN"}</b></span><span className="true"><small>TRUE PRICE</small><b>{offer.deliveryKnown ? gbp(offer.totalDeliveredGbp) : "—"}</b></span><span><small>VS RRP</small><b>{offer.deliveryKnown ? delta ?? "RRP UNKNOWN" : "—"}</b></span></div>
