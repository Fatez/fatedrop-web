...
export default async function DashboardTruePricePage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const result = q.length >= 2 ? await requestFateVerdict(q) : null;
    ...
    <div className="fd-true-price-page">
      <section className="fd-dash-card fd-true-price-hero">
        <div className="fd-tp-intro">
          <span>TRUE PRICE · FATEFIND VALUE INTELLIGENCE</span>
          <h1>The price on the shelf is not always the price at your door.</h1>
          <p>True Price answers <b>what you actually pay</b>, while RRP/reference position answers <b>whether the item itself is good value</b>. FateFind combines those measurements and FateDrop Cloud returns one canonical Fate Verdict for both Web and App.</p>
        </div>
        ...
        <form action="/dashboard/true-price" method="get" className="fd-tp-search">
          ...
          <button type="submit">RUN FATEFIND →</button>
        </form>
        <p className="fd-tp-kid-copy">A bigger bundle can cost more at checkout and still be better value. FateFind scales the verified RRP/reference basis to the correct product quantity, ranks the value position centrally in FateDrop Cloud, and keeps True Price separate so delivery never distorts the RRP percentage.</p>
      </section>

      <section className="fd-dash-card fd-tp-results">
        <div className="fd-tp-results-head"><div><span>{q ? `COMPARING · ${q.toUpperCase()}` : "COMPARE OFFERS"}</span><h2>{q ? "Which offer is cheapest — and which item is better value?" : "Search once. Compare the network."}</h2></div><small>{result ? `${result.count} product group${result.count === 1 ? "" : "s"}` : q.length >= 2 ? "Cloud verdict unavailable" : "Enter a product above"}</small></div>
        {!q ? <div className="fd-dashboard-empty"><strong>Type a product above.</strong><span>FateFind compares qualifying observed retailer offers, RRP/reference position and known checkout cost without inventing missing evidence.</span></div> : q.length < 2 ? <div className="fd-dashboard-empty"><strong>Keep typing.</strong><span>Use at least two characters so FateDrop can resolve a meaningful product search.</span></div> : !result ? <div className="fd-dashboard-empty"><strong>The canonical FateFind service could not be reached.</strong><span>No client-side fallback winner, sample prices or pretend retailers are substituted.</span></div> : groups.length ? <>
          <ValueCompare groups={groups} query={q} verdict={result.verdict} />
          <div className="fd-tp-groups">{groups.map((group) => <article className="fd-tp-group" key={group.id}>
            ...
            <footer><Link href={`/dashboard/watchlist?q=${encodeURIComponent(group.title)}`}>CREATE A FATEMATCH →</Link><Link href={`/dashboard/search?q=${encodeURIComponent(group.title)}`}>OPEN NETWORK SEARCH →</Link></footer>
          </article>)}</div>
        </> : <div className="fd-dashboard-empty"><strong>No comparable live offers found.</strong><span>FateDrop will not invent a comparison when the network has no qualifying observed offers.</span><Link className="fd-dashboard-wide-button" href={`/dashboard/watchlist?q=${encodeURIComponent(q)}`}>Create a FateMatch for this product →</Link></div>}
        {result?.disclaimer ? <p className="fd-tp-disclaimer">{result.disclaimer}</p> : null}
      </section>

      <section className="fd-dash-card fd-tp-trust"><div><span>WHY THIS MATTERS</span><h2>One value engine. Same answer everywhere.</h2></div><p>RRP/reference percentage compares item price with the verified value baseline. True Price separately tells you the real checkout cost when mandatory delivery is known. The final ranking and Fate Verdict come from FateDrop Cloud, so Web and App cannot quietly drift into different answers.</p></section>
    </div>
    <style>{`
