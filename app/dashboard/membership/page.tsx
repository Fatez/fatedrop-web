...
export default async function DashboardMembershipPage({ searchParams }: { searchParams: Promise<{ billing?: string }> }) {
  ...
  const waitingForPlus = params.billing === "success" && !premium;
  const discordLinked = Boolean(snapshot?.discord);
  ...
  return (
    <DashboardPageShell title="Membership" eyebrow="FREE OR FATEDROP PLUS">
      <div className="fd-membership-page">
        ...
        <section className="fd-dash-card fd-discord-handoff" id="discord-access">
          <div>
            <span>DISCORD ACCESS</span>
            <h2>{waitingForPlus ? "Confirm Plus, then enter Premium Discord." : premium ? "Your Plus membership carries into Discord." : "The free community stays open."}</h2>
            <p>{waitingForPlus ? "Your Stripe checkout has returned, but the signed membership webhook has not reached this page yet. Refresh the membership state before connecting Discord so FateDrop can apply the Premium role from canonical entitlement truth." : premium ? discordLinked ? discordRoleSynced ? "Your linked Discord account already has the FateDrop Premium role." : "Your Discord identity is linked. Reconcile it against the current Plus entitlement to make sure Premium access is applied." : "Connect Discord through FateDrop. OAuth joins the server and FateDrop applies the Premium role only when this FateDrop ID has an active Plus entitlement." : discordLinked ? "Your Discord identity is linked to the free community. FateDrop keeps the Premium role off until this FateDrop ID becomes eligible." : "Connect through FateDrop to join the free community. Free accounts are deliberately kept out of the Premium role."}</p>
          </div>
          <div className="fd-discord-handoff-actions">
            {waitingForPlus ? <Link className="button button-primary" href="/dashboard/membership?billing=success#discord-access">Refresh membership state <span>↻</span></Link> : premium ? discordLinked ? <DiscordSyncButton /> : <Link className="button button-primary" href="/api/discord/connect">Join Discord + unlock Premium <span>↗</span></Link> : <Link className="button button-secondary" href="/api/discord/connect">Join the free Discord community <span>↗</span></Link>}
            <Link className="text-link" href="/account">Manage Discord from your FateDrop ID <span>→</span></Link>
          </div>
        </section>

        <section className="fd-dash-card fd-membership-sync"><div><span>WHAT HAPPENS AFTER YOU UPGRADE</span><h2>One entitlement becomes available to the rest of FateDrop.</h2></div><div className="fd-membership-sync-flow"><span><b>1</b><strong>STRIPE</strong><small>Payment or trial state</small></span><i>→</i><span><b>2</b><strong>FATEDROP ID</strong><small>Authoritative entitlement</small></span><i>→</i><span><b>3</b><strong>APP</strong><small>Refreshes account access</small></span><i>→</i><span><b>4</b><strong>DISCORD</strong><small>Role reconciled from entitlement</small></span></div></section>
