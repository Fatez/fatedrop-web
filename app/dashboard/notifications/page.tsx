...
import { getLiveCloudAlertFacets, type CloudAlertFacetOptionsResponse } from "@/lib/live-signals";
...
export default async function NotificationPreferencesPage() {
  ...
  let persistent = true;
  const fallbackFacetOptions: Pick<CloudAlertFacetOptionsResponse, "languages" | "sets"> = {
    languages: [
  ...
  const [preferenceResult, liveFacetOptions] = await Promise.all([
    getNotificationPreferences(snapshot.account.id).catch(() => null),
    getLiveCloudAlertFacets().catch(() => null),
  ]);
  if (preferenceResult) preferences = preferenceResult;
  else persistent = false;
  const facetOptions = liveFacetOptions ?? fallbackFacetOptions;
  return <DashboardPageShell title="Notification Preferences" eyebrow="ONE PROFILE · EVERY CHANNEL">
    <div className="fd-notification-page">
      <section className="fd-dash-card fd-notification-hero">
        ...
        {!persistent ? <small>Apply the pending notification preference migrations before these settings can persist in production.</small> : null}
      </section>
      <section className="fd-dash-card fd-notification-editor"><div className="fd-notification-editor-head"><span>YOUR ALERT RULES</span><h2>Tell FateDrop what is worth interrupting you for.</h2><p>The same lifecycle, product, market and set record is used by supported Web and App alert delivery.</p></div><NotificationPreferenceForm initial={preferences} persistent={persistent} facetOptions={facetOptions}/></section>
    </div>
    <style>{`
