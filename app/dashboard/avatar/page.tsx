...
export default async function DashboardAvatarPage() {
  ...
  return <DashboardPageShell title="FateDrop Companion" eyebrow="ACCOUNT · COMPANION PREVIEW">
    <div className="fd-avatar-page">
      <section className="fd-avatar-hero">
        <div><span>FATEDROP // YOUR NETWORK COMPANION</span><h1>Meet the collector<br/><em>that hunts with you.</em></h1><p>Preview the current 3D Companion and Signal Droid reactions, then shape the account loadout designed to follow your FateDrop ID across web and app surfaces as the shared Companion system is completed.</p></div>
        <div className="fd-avatar-hero-tags"><span>3D PREVIEW</span><span>ACCOUNT LOADOUT</span><span>SIGNAL REACTIONS</span><span>APP + WEB FOUNDATION</span></div>
      </section>
      <Companion3DStage />
      <AvatarBuilder initialLoadout={record.loadout} initialFavouriteTcgs={record.favouriteTcgs} persistent={persistent}/>
      <section className="fd-avatar-usage"><article><span>01</span><strong>DASHBOARD</strong><p>The Companion preview is isolated from the rest of the collector command centre so a 3D asset problem cannot block Search, Alerts or retailer journeys.</p></article><article><span>02</span><strong>ALERTS + FATEFIND</strong><p>The same Companion vocabulary reacts to Watching, Echo, Manifested, Vanished, FateMatch and major signal states without changing the underlying evidence.</p></article><article><span>03</span><strong>ROLE-LOCKED 3D</strong><p>Companion assets are validated by role before rendering, so a card, prop, droid or humanoid cannot silently appear in the wrong slot.</p></article><article><span>04</span><strong>APP + WEB</strong><p>The website and mobile app follow the same Companion identity and loadout vocabulary, ready to converge on one account-selected character.</p></article></section>
    </div>
    <style>{`
