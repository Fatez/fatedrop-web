...
import { Companion3DStage } from "@/components/companion-3d-stage";
...
export default async function DashboardAvatarPage() {
  ...
  return <DashboardPageShell title="FateDrop Companion" eyebrow="ACCOUNT · LIVE COMPANION">
    <div className="fd-avatar-page">
      <section className="fd-avatar-hero">
        <div><span>FATEDROP // YOUR NETWORK COMPANION</span><h1>Meet the collector<br/><em>that hunts with you.</em></h1><p>Your FateDrop Companion now has a real 3D layer on the website as well as the mobile foundation. Choose a character or Signal Droid, preview the core FateDrop reactions, then keep using the account loadout builder below for the identity data that will remain shared across surfaces.</p></div>
        <div className="fd-avatar-hero-tags"><span>LIVE 3D</span><span>ACCOUNT LOADOUT</span><span>SIGNAL REACTIONS</span><span>APP + WEB</span></div>
      </section>
      <Companion3DStage />
      <AvatarBuilder initialLoadout={record.loadout} initialFavouriteTcgs={record.favouriteTcgs} persistent={persistent}/>
      <section className="fd-avatar-usage"><article><span>01</span><strong>DASHBOARD</strong><p>Your Companion is now a live 3D presence in the collector command centre, not just an illustrated placeholder.</p></article><article><span>02</span><strong>ALERTS + FATEFIND</strong><p>The same Companion can react to Watching, Echo, Manifested, Vanished, FateMatch and major signal states without changing the underlying evidence.</p></article><article><span>03</span><strong>3D LAYER</strong><p>Male, female and Signal Droid variants use lightweight coloured geometry so the feature stays responsive instead of shipping the original 100MB+ source models.</p></article><article><span>04</span><strong>APP + WEB</strong><p>The website and mobile app now follow the same Companion vocabulary and asset family, ready to converge on one account-selected identity.</p></article></section>
    </div>
    <style>{`
