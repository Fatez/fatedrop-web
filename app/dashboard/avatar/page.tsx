  ...
  return <DashboardPageShell title="FateDrop Companion" eyebrow="ACCOUNT · COMPANION FOUNDATION">
    <div className="fd-avatar-page">
      <section className="fd-avatar-hero">
        <div><span>FATEDROP // YOUR NETWORK COMPANION</span><h1>Build the collector<br/><em>that hunts with you.</em></h1><p>Your FateDrop Companion belongs to your account, not a single page. The Sentinel 3D renderer is now wired into the Companion layer with the illustrated character retained as a safe fallback. Your account loadout remains the source of truth while modular 3D cosmetics, rigged reactions and the floating signal droid become the next layer.</p></div>
        <div className="fd-avatar-hero-tags"><span>ACCOUNT LOADOUT</span><span>ORIGINAL COSMETICS</span><span>SIGNAL REACTIONS</span><span>3D SENTINEL READY</span></div>
      </section>
      <AvatarBuilder initialLoadout={record.loadout} initialFavouriteTcgs={record.favouriteTcgs} persistent={persistent}/>
      <section className="fd-avatar-usage"><article><span>01</span><strong>DASHBOARD</strong><p>Your Companion is the personal presence in your collector command centre.</p></article><article><span>02</span><strong>ALERTS + FATEFIND</strong><p>The same Companion can react to Watching, Whisper, Manifested, Vanished and Echo states without changing the underlying evidence.</p></article><article><span>03</span><strong>3D LAYER</strong><p>The Sentinel GLB is wired into the account renderer with automatic illustrated fallback. The current model is static; rigged clips and modular cosmetics remain the next asset step.</p></article><article><span>04</span><strong>APP</strong><p>The loadout is account data, ready for the mobile app to consume instead of creating a second Companion.</p></article></section>
    </div>
    <style>{`
