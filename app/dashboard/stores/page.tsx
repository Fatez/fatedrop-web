...
export default async function DashboardStoresPage() {
  ...
  return <DashboardPageShell title="Indies & Retailers" eyebrow="ONE NEUTRAL RETAILER NETWORK">
    <div className="fd-retailer-hub">
      <section className="fd-dash-card fd-retailer-hero">
        <div className="fd-dash-card-head"><span>RETAILER NETWORK · EVIDENCE FIRST</span><i className={cloud.some((item) => item.runtime.healthy) ? "live" : "pending"}>{cloud.length ? `● ${cloud.length} CLOUD RETAILER${cloud.length === 1 ? "" : "S"}` : "○ AWAITING CLOUD"}</i></div>
        <div className="fd-retailer-message"><h1>Big retailers give context.<br/>Indies give collectors more choice.</h1><p>FateDrop keeps national/RRP comparison retailers and independent businesses visibly distinct while letting both contribute useful offer evidence. A connected Indie can be discovered through Search and FateFind when its genuine offer qualifies, then the collector still buys directly from that store. <strong>Retailers cannot pay for a better FateFind verdict, artificial trust or RRP treatment.</strong></p></div>
        <div className="fd-retailer-metrics"><div><strong>{major.length}</strong><span>RRP / NATIONAL</span><small>Reference and comparison lane</small></div><div><strong>{indies.length + storefronts.length}</strong><span>INDEPENDENTS</span><small>Specialist + connected discovery</small></div><div><strong>{indies.filter((item) => item.physicalStores === true).length + storefronts.filter((item) => item.physicalStores).length}</strong><span>PHYSICAL</span><small>Only where location evidence exists</small></div></div>
      </section>

      <RetailerMarketDirectory retailers={retailerNetwork} labStorefronts={storefronts} />

      <section className="fd-dash-card fd-retailer-model"><div><span>WHY INDIES MATTER</span><h2>More connected stores can mean more real buying opportunities.</h2></div><p>Small retailers often have useful stock without the reach of a national chain. FateDrop can bridge that gap: their connected offers become discoverable, collectors get more places to buy, and the retailer keeps its own website, checkout, fulfilment and customer relationship.</p></section>
      <section className="fd-dash-card fd-retailer-model"><div><span>HOW FATEDROP STAYS NEUTRAL</span><h2>Participation can improve coverage. Payment cannot buy the answer.</h2></div><p>A paid FateDrop Indie membership is for catalogue connection, storefront control, analytics and business tools. Organic Search visibility and FateFind value decisions must continue to come from genuine stock, product identity and price evidence.</p></section>
    </div>
    <style>{`.fd-retailer-hub{display:grid;gap:12px;max-width:1600px;margin:0 auto}.fd-retailer-hub .fd-dash-card{border-color:rgba(221,203,188,.085);border-radius:12px;background:linear-gradient(145deg,#0f1317,#090d11 74%)}.fd-retailer-hero{padding:28px;overflow:hidden;background:radial-gradient(circle at 90% 8%,rgba(126,87,143,.14),transparent 28%),linear-gradient(145deg,#101419,#090d11 70%)!important}.fd-retailer-message h1{max-width:900px;margin:18px 0 12px;color:#eee4dc;font-family:Georgia,'Times New Roman',serif;font-size:clamp(2.5rem,4vw,4.7rem);font-weight:500;line-height:.96;letter-spacing:-.05em}.fd-retailer-message p{font-size:14px;line-height:1.75;max-width:980px;color:#a0989b}.fd-retailer-message strong{color:#cbb09d;font-size:.88em;letter-spacing:.015em}.fd-retailer-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:24px}.fd-retailer-metrics div{padding:15px;border:1px solid rgba(221,203,188,.065);border-radius:10px;background:rgba(255,255,255,.015)}.fd-retailer-metrics strong{display:block;color:#eee4dc;font-family:Georgia,serif;font-size:28px;font-weight:500}.fd-retailer-metrics span{display:block;margin-top:4px;color:#b6977d;font-size:10px;font-weight:900;letter-spacing:.11em}.fd-retailer-metrics small{display:block;margin-top:4px;color:#81797d;font-size:10px}.fd-retailer-model{padding:24px 28px;display:grid;grid-template-columns:minmax(0,.9fr) minmax(0,1.1fr);gap:32px;align-items:center}.fd-retailer-model span{font-size:10px;letter-spacing:.14em;color:#b6977d;font-weight:900}.fd-retailer-model h2{margin:7px 0 0;color:#e4dad2;font-family:Georgia,serif;font-size:24px;font-weight:500;line-height:1.15}.fd-retailer-model p{margin:0;color:#989095;font-size:13px;line-height:1.7}@media(max-width:850px){.fd-retailer-hero{padding:20px}.fd-retailer-metrics,.fd-retailer-model{grid-template-columns:1fr}}`}</style>
  </DashboardPageShell>;
}
