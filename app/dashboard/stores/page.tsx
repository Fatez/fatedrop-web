...
export default async function DashboardStoresPage() {
  ...
  const storefronts = [
    { id: "wishlist-collectables", name: "Wishlist Collectables", location: "London · Online + physical", tags: ["Pokémon", "Lorcana", "One Piece", "Yu-Gi-Oh"], products: wishlist, href: "/dashboard/stores/wishlist-collectables", note: "Experimental storefront", image: WISHLIST_BRAND_IMAGE, imageMode: "cover" },
    { id: "cob-and-pip", name: "Cob & Pip", location: "UK · Online", tags: ["Pokémon", "TCG"], products: cobAndPip, href: "/dashboard/stores/cob-and-pip", note: "Experimental storefront", image: COB_AND_PIP_LOGO, imageMode: "contain" },
  ];
    ...
    <div className="fd-indies-page">
      <section className="fd-dash-card fd-indies-hero">
        ...
        <div className="fd-indies-stats">
          ...
          <div><strong>{indexed || "—"}</strong><span>LAB STOREFRONT ITEMS</span><small>Experimental direct feeds only</small></div>
        </div>
      </section>
      ...
      <section className="fd-dash-card fd-indies-network">
        ...
        </article>)}</div> : <div className="fd-dashboard-empty"><strong>Retailer network health is unavailable.</strong><span>FateDrop does not infer “live” status from a logo or static configuration.</span></div>}
      </section>
      ...
      <section className="fd-dash-card fd-indies-lab">
        ...
        <div className="fd-indies-store-grid">{storefronts.map((store) => { const inStock = store.products.filter((product) => product.available).length; return <article key={store.id}>
          <div className="fd-indies-store-image" style={{ backgroundImage: `url("${store.image}")`, backgroundSize: store.imageMode }} />
          <div className="fd-indies-store-body"><small>{store.note}</small><h3>{store.name}</h3><p>{store.location}</p><div className="fd-indies-tags">{store.tags.map((tag)=><span key={tag}>{tag}</span>)}</div><div className="fd-indies-store-metrics"><span><b>{store.products.length}</b> indexed</span><span><b>{inStock}</b> observed available</span></div><Link href={store.href}>Explore storefront preview →</Link></div>
        </article>; })}</div>
      </section>
