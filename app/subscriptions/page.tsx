...
export const metadata: Metadata = {
  ...
  description: "Explore FateDrop Plus for collectors and the founding FateDrop Indie retailer network.",
};
...
function RetailerPlanGrid({ plans }: { plans: readonly RetailerPlan[] }) {
  return <div className="plan-grid">{plans.map((plan) => <article className={`plan-card${"featured" in plan && plan.featured ? " featured" : ""}`} key={plan.name}>{"featured" in plan && plan.featured ? <span className="price-badge">Founding indie network</span> : null}<p>{plan.name}</p><h3>{plan.price}</h3>{"secondaryPrice" in plan && plan.secondaryPrice ? <small>{plan.secondaryPrice}</small> : <small>Organic retailer discovery remains free</small>}<ul>{plan.features.map((feature) => <li key={feature}>{feature}</li>)}</ul><Link href="/join?type=business">Connect your catalogue <b>↗</b></Link></article>)}</div>;
}
  ...
  return (
    <SiteShell>
      <MarketStoryHero
        ...
        focal="right"
      >
        <div className="button-row"><Link className="button button-primary" href="#collectors">Collector membership <span>↓</span></Link><Link className="button button-secondary" href="#retailers">Retailer membership</Link></div>
      </MarketStoryHero>
      ...
      <section className="plan-section section-shell" id="retailers"><SectionHeading eyebrow="Independent retailer membership" title="Give small shops a route into collector demand." body="A retailer does not pay to become more trusted or to outrank a better deal. Free verified retailers can remain organically discoverable; FateDrop Indie pays for deeper catalogue connection, storefront control and useful retailer tools while checkout stays with the shop." /><RetailerPlanGrid plans={siteConfig.retailerPlans} /></section>
      <section className="content-section section-shell"><div className="quote-band"><p className="eyebrow"><span />Two sides of one network</p><blockquote>Help collectors find value. Help good independent shops be found.</blockquote><p>Consumer subscription revenue funds the intelligence and delivery layer. Retailer subscription revenue funds catalogue connection and business tools. A subscription cannot buy a false RRP verdict, alert priority or better organic ranking.</p><div className="button-row" style={{ marginTop: 30 }}><Link className="button button-primary" href="/account/register">Create your FateDrop ID <span>↗</span></Link><Link className="button button-secondary" href="/join?type=business">Join as an indie retailer</Link></div></div></section>
    </SiteShell>
  );
}
