  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!seriesResult.payload.ok || !cardsResult.payload.ok) {
        const dark = seriesResult.response.status === 404 && seriesResult.payload.error?.code === "NOT_FOUND";
        setBackendAvailable(!dark);
