...
const NETWORK_SNAPSHOT_FRESH_SECONDS = 15 * 60;
...
export async function DashboardNetworkPulse({ retailers, products, signals }: DashboardNetworkPulseProps) {
  const latestSnapshot = await getLatestNetworkMetricSnapshot().catch(() => null);
  const now = Math.floor(Date.now() / 1000);
  const snapshotAgeSeconds = latestSnapshot ? Math.max(0, now - latestSnapshot.measuredAt) : null;
  const snapshotFresh = snapshotAgeSeconds !== null && snapshotAgeSeconds <= NETWORK_SNAPSHOT_FRESH_SECONDS;
  const visibleRetailers = snapshotFresh ? retailers : null;
  const visibleProducts = snapshotFresh ? products : null;
  ...
  return (
    <div className="fd-pulse-layout">
      <div className="fd-pulse-map" role="img" aria-label="Illustrative FateDrop United Kingdom network artwork">
        <img src="/assets/dashboard/network-pulse-map.svg" alt="" />
      </div>
