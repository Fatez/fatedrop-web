...
export function IndieStorefront({ products, retailerName }: { products: CatalogueProduct[]; retailerName: string }) {
  ...
  const visible = useMemo(() => products
    ...
    .filter((product) => catalogueProductMatches(product, query))
  ...
  return <section className="fd-dash-card fd-indie-storefront">
    ...
    <p className="fd-indie-store-intro">Browse everything FateDrop has indexed for {retailerName}. Search understands common TCG shorthand such as ETB, PC ETB, booster box and 3-pack.</p>
    <label className="fd-indie-search"><span>⌕</span><div><small>SEARCH {retailerName.toUpperCase()}</small><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try ETB, booster box, Destined Rivals…" /></div>{query ? <button type="button" onClick={() => setQuery("")}>×</button> : null}</label>
    ...
    </article>)}</div> : <div className="fd-dashboard-empty"><strong>No matching products.</strong><span>Try another product name, set name or TCG shorthand.</span></div>}
    <style jsx>{`
