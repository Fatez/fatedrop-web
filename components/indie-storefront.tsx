...
export function IndieStorefront({ products, retailerName }: { products: CatalogueProduct[]; retailerName: string }) {
  ...
  return <section className="fd-dash-card fd-indie-storefront">
    <div className="fd-dash-card-head"><span>STORE CATALOGUE</span><i className="live">● {products.length} ITEMS INDEXED</i></div>
    ...
    {visible.length ? <div className="fd-dashboard-list fd-indie-products">{visible.map((product) => <article key={product.id}>
      ...
      <aside><span>{formatGBP(product.pricePence)}</span><a href={product.url} target="_blank" rel="noreferrer">VIEW ↗</a></aside>
    ...
    <style jsx>{`
      .fd-indie-storefront{padding:28px}.fd-indie-search{min-height:70px;margin:20px 0 12px;padding:10px 14px;display:grid;grid-template-columns:34px 1fr auto;align-items:center;gap:12px;border:1px solid rgba(88,232,255,.24);border-radius:18px;background:linear-gradient(135deg,rgba(88,232,255,.07),rgba(157,109,255,.06));transition:.2s ease}.fd-indie-search:focus-within{border-color:rgba(88,232,255,.58);box-shadow:0 0 0 3px rgba(88,232,255,.06)}.fd-indie-search>span{font-size:28px;color:#68e8fb}.fd-indie-search>div{display:flex;min-width:0;flex-direction:column;gap:4px}.fd-indie-search small{font-size:9px;letter-spacing:.16em;color:#817b89;font-weight:850}.fd-indie-search input{border:0;outline:0;background:transparent;color:#fff;font-size:16px;font-weight:650}.fd-indie-search input::placeholder{color:#746f7a}.fd-indie-search button{width:34px;height:34px;border:1px solid rgba(255,255,255,.12);border-radius:10px;background:rgba(255,255,255,.05);color:#fff;font-size:20px;cursor:pointer}.fd-indie-controls{display:flex;gap:8px;margin-bottom:18px}.fd-indie-controls button{padding:10px 14px;border:1px solid rgba(255,255,255,.1);border-radius:11px;background:rgba(255,255,255,.03);color:#96909e;font-size:11px;font-weight:750;cursor:pointer}.fd-indie-controls button[data-active="true"]{border-color:rgba(157,109,255,.35);background:rgba(157,109,255,.12);color:#fff}.fd-indie-products article{min-height:82px}.fd-indie-products strong{font-size:14px}.fd-indie-products small{font-size:11px}.fd-indie-products aside>span{font-size:15px;font-weight:850;color:#fff}.fd-indie-products aside a{font-size:9px;font-weight:850}@media(max-width:700px){.fd-indie-storefront{padding:20px}.fd-indie-search{grid-template-columns:28px 1fr auto}.fd-indie-search input{font-size:14px}}
    `}</style>
  </section>;
}
