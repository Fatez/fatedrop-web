...
export function NotificationPreferenceForm({ initial, persistent }: { initial: NotificationPreferences; persistent: boolean }) {
  ...
  const signals = [["echo","Echo","Early meaningful network movement"],["manifested","Manifested","Confirmed availability/restock"],["vanished","Vanished","Previously confirmed availability lost"],["priceChange","Price changes","Observed price movement"],["fateMatch","FateMatch","A result satisfies one of your FateFinds"]] as const;
  ...
  return <div className="fd-pref-editor">
    ...
    <div className="fd-pref-save"><button type="button" onClick={save} disabled={!persistent || status === "saving"}>{!persistent ? "MIGRATION REQUIRED" : status === "saving" ? "SAVING…" : status === "saved" ? "✓ SAVED" : status === "error" ? "TRY AGAIN" : "SAVE PREFERENCES"}</button><small>These are preference records only. A channel is not advertised as operational until its delivery integration is actually connected.</small></div>
