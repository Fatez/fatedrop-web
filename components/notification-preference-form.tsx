...
export function NotificationPreferenceForm({ initial, persistent, facetOptions }: { initial: NotificationPreferences; persistent: boolean; facetOptions: FacetOptions }) {
  ...
  return <div className="fd-pref-editor">
    ...
    <section><span>MANIFESTED AVAILABILITY REMINDERS</span><div className="fd-pref-intro"><strong>Keep useful confirmed stock visible—without pretending it is a new drop.</strong><small>Optional reminders only reuse a still-open canonical stock episode after fresh re-confirmation. They are off by default and never create another Manifested event.</small></div><label><div><strong>Still available reminders</strong><small>Receive a restrained “Still observed available” reminder when no natural alert has arrived recently.</small></div><input type="checkbox" checked={preferences.manifestedReminders} onChange={()=>toggle("manifestedReminders")}/></label>{preferences.manifestedReminders?<label><div><strong>Daily reminder cap</strong><small>Maximum engagement reminders per rolling 24 hours.</small></div><select value={preferences.manifestedRemindersMaxPerDay} onChange={(event)=>setPreferences((current)=>({...current,manifestedRemindersMaxPerDay:Number(event.target.value)}))}><option value={1}>1 per day</option><option value={2}>2 per day</option><option value={3}>3 per day</option></select></label>:null}</section>
