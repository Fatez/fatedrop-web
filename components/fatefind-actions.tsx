...
export function FateFindActions({ id, enabled }: { id: string; enabled: boolean }) {
  ...
  const fateFindQuery = latestHit?.query || latestHit?.productTitle || "";
  ...
  return <div className="fd-fatefind-actions">
    ...
    {latestHit ? <div className="fd-fatefind-hit">
      ...
      <div>
        ...
        {fateFindQuery ? <a href={`/dashboard/fatefind?q=${encodeURIComponent(fateFindQuery)}`}>FATEFIND →</a> : null}
      </div>
    </div> : null}
    {message ? <small className="fd-fatefind-message">{message}</small> : null}
    <style jsx>{`
      .fd-fatefind-actions{min-width:170px;display:grid;gap:7px}.fd-fatefind-controls{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap}.fd-fatefind-actions button{min-height:28px;padding:0 9px;border:1px solid rgba(171,126,195,.16);border-radius:7px;background:rgba(112,72,140,.05);color:#b895c5;font-size:11px;font-weight:900;letter-spacing:.07em}.fd-fatefind-actions button.danger{border-color:rgba(179,83,91,.16);background:rgba(179,83,91,.035);color:#b9797e}.fd-fatefind-actions button:disabled{opacity:.48}.fd-fatefind-hit{padding:8px;border:1px solid rgba(118,158,103,.16);border-radius:8px;background:rgba(99,137,83,.045);display:grid;gap:3px}.fd-fatefind-hit>b{color:#9eb991;font-size:10px;letter-spacing:.08em}.fd-fatefind-hit>span{color:#d5ccc5;font-size:12px}.fd-fatefind-hit>small{color:#746e6d;font-size:10px;text-transform:capitalize}.fd-fatefind-hit>div{display:flex;gap:8px;flex-wrap:wrap;margin-top:3px}.fd-fatefind-hit a{color:#ba94c9;font-size:11px;font-weight:900;text-decoration:none}.fd-fatefind-message{color:#746d6f;font-size:10px;text-align:right}
    `}</style>
  </div>;
}
