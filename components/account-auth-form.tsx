  ...
  return (
    <form className="identity-auth-form" onSubmit={submit} noValidate>
      ...
      <button className="button button-primary" type="submit" disabled={busy || !turnstileReady}>
        {busy ? mode === "register" ? "Sending request…" : "Connecting…" : mode === "register" ? "Request closed beta access" : "Enter the network"} <span>↗</span>
      </button>
      <p className="identity-auth-switch">
        {mode === "register"
          ? <>Already requested access or have a FateDrop ID? <Link href={`/account/login${nextQuery}`}>Sign in</Link>.</>
          : <>New to FateDrop? <Link href={`/account/register${nextQuery}`}>Request closed beta access</Link>.</>}
      </p>
      <style>{`
