...
type PairRequestState = {
  key: string;
  verdict: FatePairVerdict | null;
  error: string;
};

export function ValueCompare({ groups, query, verdict }: { groups: SignalTruePriceGroup[]; query: string; verdict: FateRankVerdict }) {
  const options = useMemo(() => groups.filter((group) => group.offers.length > 0), [groups]);
  const [leftId, setLeftId] = useState(options[0]?.id ?? "");
  const [rightId, setRightId] = useState(options[1]?.id ?? options[0]?.id ?? "");
  const [requestState, setRequestState] = useState<PairRequestState>({ key: "", verdict: null, error: "" });

  const leftGroup = options.find((group) => group.id === leftId) ?? options[0];
  const rightGroup = options.find((group) => group.id === rightId && group.id !== leftGroup?.id)
    ?? options.find((group) => group.id !== leftGroup?.id)
    ?? options[0];
  const effectiveLeftId = leftGroup?.id ?? "";
  const effectiveRightId = rightGroup?.id ?? "";
  const requestKey = effectiveLeftId && effectiveRightId && effectiveLeftId !== effectiveRightId
    ? `${query.trim()}::${effectiveLeftId}::${effectiveRightId}`
    : "";

  useEffect(() => {
    if (!requestKey || query.trim().length < 2) return;
    const controller = new AbortController();
    void fetch("/api/fatefind/verdict", {
      method: "POST",
      headers: { Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify({ query, leftId: effectiveLeftId, rightId: effectiveRightId }),
      ...
      setRequestState({ key: requestKey, verdict: data.pairVerdict ?? null, error: "" });
    }).catch((requestError: unknown) => {
      if (requestError instanceof DOMException && requestError.name === "AbortError") return;
      setRequestState({ key: requestKey, verdict: null, error: "FateDrop Cloud could not return this head-to-head verdict." });
    });
    return () => controller.abort();
  }, [effectiveLeftId, effectiveRightId, query, requestKey]);

  if (options.length < 2 || !leftGroup || !rightGroup) return null;
  const pairVerdict = requestState.key === requestKey ? requestState.verdict : null;
  const pairError = requestState.key === requestKey ? requestState.error : "";
  const loading = Boolean(requestKey) && requestState.key !== requestKey;
  ...
  return <section className="fd-value-compare">
    ...
    <div className="fd-value-selectors">
      <label><span>ITEM A</span><select value={effectiveLeftId} onChange={(event) => setLeftId(event.target.value)}>{options.map((group) => <option key={group.id} value={group.id}>{group.title}</option>)}</select></label>
      <b>VS</b>
      <label><span>ITEM B</span><select value={effectiveRightId} onChange={(event) => setRightId(event.target.value)}>{options.map((group) => <option key={group.id} value={group.id}>{group.title}</option>)}</select></label>
    </div>
    ...
    </> : <div className="fd-value-loading">{pairError || "Waiting for the canonical FateDrop Cloud verdict."}</div>}
    <style jsx>{`
