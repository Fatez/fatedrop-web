...
test("collector page explains Fate Network buying tools and Fate Trader separately", async () => {
  const page = await source("app/collectors/page.tsx");
  assert.ok(page.includes("Fate Network · For collectors"));
  assert.ok(page.includes("One network. More ways to find what you collect."));
  assert.ok(page.includes("The connected layer underneath the collector tools."));
  assert.ok(page.includes("Search tells you what FateDrop currently knows. FateFind helps decide what is strongest value now. FateMatch handles the waiting."));
  assert.ok(page.includes("A smaller retailer can still be the right answer."));
  assert.ok(page.includes("Buying and trading are different journeys."));
  assert.ok(page.includes("Fate Trade Finder"));
  assert.ok(page.includes("Fate Trade Found"));
  assert.ok(page.includes("Fate Trade Hunt"));
...
test("retailer page explains the same Fate Network from the supply side without Fate Trader clutter", async () => {
  ...
  assert.ok(page.includes("Future paid retailer tools still cannot buy stronger verification, a better RRP verdict, alert priority or organic FateFind placement."));
  assert.equal(page.includes("Fate Trade Finder"), false);
  assert.equal(page.includes("Fate Trade Hunt"), false);
});
...
test("founding retailer beta does not publish an unconfirmed paid retailer tier", async () => {
  const [siteData, subscriptions, businesses] = await Promise.all([
