...
test("Turnstile secrets remain server-only while the public site key comes from runtime configuration", () => {
  ...
  assert.doesNotMatch(loginPageSource, /NEXT_PUBLIC_TURNSTILE/);
  assert.doesNotMatch(registerPageSource, /NEXT_PUBLIC_TURNSTILE/);
});
...
test("registration keeps rate limiting, bounded bodies and generic account conflicts", () => {
...
test("Turnstile validates one-use challenge context and fails closed in production", () => {
