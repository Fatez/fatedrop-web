...
test("Turnstile secrets remain server-only while the public site key comes from runtime configuration", () => {
  ...
  assert.match(closedBetaPageSource, /process\.env\.TURNSTILE_SITE_KEY/);
  assert.match(closedBetaPageSource, /AccountAuthForm mode="register"/);
  assert.match(legacyRegisterPageSource, /redirect\("\/closed-beta"\)/);
  assert.doesNotMatch(legacyRegisterPageSource, /TURNSTILE_SITE_KEY|AccountAuthForm/);
