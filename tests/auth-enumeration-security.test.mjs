...
test("login bounds password input before running the password KDF while retaining auth abuse controls", () => {
  assert.match(loginSource, /payload\.password\.length <= 200/);
  assert.match(loginSource, /checkAuthRateLimit\(request, "login"\)/);
  assert.match(loginSource, /readBoundedJson\(request\)/);
